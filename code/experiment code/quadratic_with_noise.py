...
y_clean = x**2
# Add noise to y
noise_level = 2.0  # Adjust this to control noise amount
...
print(f"Xi correlation: {xi_scalar:.3f}")       # May be close to 0
print(f"Chatterjee's CC: {cc:.3f}")     # Should detect dependence (>0)
...
plt.plot(x, y_clean, 'r-', linewidth=2, label='y = x² (clean)')

plt.xlabel('X values')
plt.ylabel('Y values')
plt.title(f'Quadratic Relationship with Noise: y = x² + N(0, {noise_level})\nXi: {xi_scalar:.3f}, Chatterjee\'s CC: {cc:.3f}, Normalized CC: {norm_cc:.3f}\nPearson: {pearson_corr:.3f}, Spearman: {spearman_corr:.3f}')
plt.legend()
